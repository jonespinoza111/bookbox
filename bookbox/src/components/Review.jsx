...
const Review = ({ review }) => {
  ...
  return (
    <div
      className={`bg-white p-4 rounded shadow my-4 relative ${
        needsThumbnail && "flex md:flex-row flex-col"
      }`}
    >
      <div className="mr-4 mb-4 md:mb-0">
        {bookInfo && needsThumbnail && (
          <BookThumbnail
            ...
            thumbnail={bookInfo.smallThumbnail}
            size="small"
          />
        )}
      </div>
      <div>
        <h3 className="text-md sm:text-lg text-gray-500 mb-2">
          {review.username}
        </h3>
        ...
        {userInfo && userInfo.attributes.sub === review.author && (
          <button
            className="absolute bottom-0 right-0 p-4 cursor-pointer hover:bg-gray-400 text-black"
            onClick={handleEdit}
          >
            Edit
          </button>
