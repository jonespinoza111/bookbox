...
const Book = ({ id, title, author, thumbnail, size = "medium" }) => {
  ...
  return (
    <div className={`bg-white ${size == "medium" && 'w-[8em] h-[16em]'} rounded-lg shadow-md ${size == "small" && 'w-[6em] h-[10em]'} cursor-pointer`} title="See Details" onClick={() => navigate(`/book/${id}`)}>
      <div className={`w-[100%] ${size == "medium" && 'h-[12em]'} ${size == "small" && 'h-[7.5em]'}`}>
