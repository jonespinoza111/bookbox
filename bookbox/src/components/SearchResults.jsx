  ...
  return (
    <div className="flex flex-row flex-wrap px-[5em] gap-x-3 gap-y-4">
      {results && results.map((result) => (
        <div key={result.id} className="h-[240px] w-[160px] rounded-md cursor-pointer relative" title="view details" onClick={() => navigate(`/book/${result.id}`)}>
          <img src={result.thumbnail} alt={result.title} className="w-full h-full" />
